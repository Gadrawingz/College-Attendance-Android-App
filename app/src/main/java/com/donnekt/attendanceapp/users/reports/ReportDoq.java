...
public class ReportDoq extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_report_doq);
        ...
        if(STAFF_ROLE.equals("DPAT")) {
            mainTitle.setText("DOQ Attendance Reports");
        } else {
            mainTitle.setText("HOD Attendance Reports");
        }

        subTitle.setText("Reports from all departments");

        mainTitle.setOnClickListener(v -> {
            Intent i = new Intent(ReportDoq.this, StaffMenus.class);
    ...
    private void showHodReportData() {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, URLs.REPORT_DOQ_HODs, response -> {
            progressBar.setVisibility(View.INVISIBLE);
            findViewById(R.id.mainTitle).setVisibility(View.VISIBLE);
            try {
                JSONObject object = new JSONObject(response);
                JSONArray dataArray = object.getJSONArray("doq_rep_hods");
                ...
                for (int i = 0; i < dataArray.length(); i++) {
                    ...
                    Report report = new Report(
                            ...
                            dataObject.getString("class_counts")
                    );

                    hodReportList.add(report);
                    DoqReportAdapter adapter = new DoqReportAdapter(hodReportList, getApplicationContext());
